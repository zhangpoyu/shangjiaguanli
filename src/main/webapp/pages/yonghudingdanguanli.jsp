		...
		<div class="wrapper">
			...
			<div class="container_full">
				...
				<main class="content_wrapper">
                    ...
                    <c:if test="${ !empty dingdanall}">
							<c:forEach items="${ dingdanall}" var="item">
                      ...
                      </td><td>                        ${item.zhuangtai }
                      </td><td>                        ${item.jiage }
                      </td><td>                        ${item.shuliang }
                      </td><td>                        ${item.shangjia }
                      </td><td>                        ${item.shangjiaid }
                      </td><td>
                          <a class="table-actions" href="shangpingxiangqing.action?id=${item.shangpingid}">查看</a>
                          
                          <c:if test="${item.zhuangtai == '配送中'}">
                          <a class="table-actions" href="shouhuodingdan.action?id=${item.id }">签收</a>  
                          </c:if>
                          <c:if test="${item.zhuangtai == '已签收'}">
                          <a class="table-actions" href="yonghutianjiapingjia.action?shangpingid=${item.shangpingid}&shanghuid=${item.shangjiaid }">评价</a>  
                          </c:if>
                           
                           
                           
                           
                            <a class="table-actions" href="shanchudingdan.action?id=${item.id}">删除</a>
                      </td>                    </tr>
							</c:forEach>
				</c:if>
                  </tbody>
